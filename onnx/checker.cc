...
void check_opset_compatibility(
  ...
  if (!schema_for_model_import || !schema_for_function_import ||
    ...
    fail_check(
        ...
        onnx::to_string(func_opset_version) + " whereas model imports version " +
        onnx::to_string(model_opset_version));
  }
}
...
void check_model(const std::string& model_path, bool full_check, bool skip_opset_compatibility_check) {
  ...
  if (full_check) {
    ...
    onnx::shape_inference::InferShapes(model, ctx.get_schema_registry(), options);
  }
}
...
void check_model(const ModelProto& model, bool full_check, bool skip_opset_compatibility_check) {
  ...
  if (full_check) {
    ...
    onnx::shape_inference::InferShapes(copy, ctx.get_schema_registry(), options);
  }
}
