...
class SchemaError final : public std::runtime_error {
 public:
  ...
  void AppendContext(const std::string& context) {
    expanded_message_ = onnx::MakeString(std::runtime_error::what(), "\n\n==> Context: ", context);
  }
...
#define fail_schema(...) ONNX_THROW_EX(onnx::SchemaError(onnx::MakeString(__VA_ARGS__)));
...
#define ONNX_OPERATOR_SCHEMA_UNIQ(Counter, name)                                                                      \
  static onnx::OpSchemaRegistry::OpSchemaRegisterOnce(op_schema_register_once##name##Counter) ONNX_UNUSED = \
