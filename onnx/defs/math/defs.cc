...
void matmulShapeInference(onnx::InferenceContext& ctx, int input1Idx, int input2Idx) {
  ...
  onnx::TensorShapeProto shapeL, shapeR;
  ...
  onnx::TensorShapeProto resultShape;

  // Now call out to generic multidimensional broadcasting for
  // the broadcastable prefixes.
  {
    onnx::TensorShapeProto prefixShapeL, prefixShapeR;
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction([](onnx::InferenceContext& ctx) {
          auto a_type = ctx.getInputType(0);
          auto b_type = ctx.getInputType(3);
          if (nullptr == a_type || nullptr == b_type ||
              a_type->value_case() != onnx::TypeProto::kTensorType ||
              b_type->value_case() != onnx::TypeProto::kTensorType) {
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction([](onnx::InferenceContext& ctx) {
          ...
          y_type->mutable_tensor_type()->set_elem_type(onnx::TensorProto::INT32);
...
void einsumRankInference(onnx::InferenceContext& ctx, std::string equation) {
...
template <typename T>
static T get_scalar_value_from_tensor(const onnx::TensorProto* t) {
  if (t == nullptr) {
    return T{};
  }
  ...
  switch (data_type) {
    case onnx::TensorProto::FLOAT:
      return static_cast<T>(onnx::ParseData<float>(t).at(0));
    case onnx::TensorProto::DOUBLE:
      return static_cast<T>(onnx::ParseData<double>(t).at(0));
    case onnx::TensorProto::INT32:
      return static_cast<T>(onnx::ParseData<int32_t>(t).at(0));
    case onnx::TensorProto::INT64:
      return static_cast<T>(onnx::ParseData<int64_t>(t).at(0));
    default:
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction([](onnx::InferenceContext& ctx) {
          ...
          auto& input_shape = getInputShape(ctx, 0);
          onnx::TensorShapeProto result_shape_proto = input_shape;
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction([](onnx::InferenceContext& ctx) {
          ...
          onnx::TensorShapeProto result_shape_proto;
