...
bool BuildContextDependentFunctionBodyLayerNormalization(
  ...
  int64_t U =
      (type_attr != nullptr) ? type_attr->i() : static_cast<int64_t>(onnx::TensorProto_DataType_FLOAT);
  if ((U != onnx::TensorProto_DataType_FLOAT) && (U != onnx::TensorProto_DataType_BFLOAT16))
    return false; // Error
  ...
  auto mktensor = [](int64_t val) -> onnx::TensorProto {
    auto tp = onnx::ToTensor(std::vector<int64_t>{val});
    tp.add_dims(1);
    return tp;
  };
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr(
            ...
            AttributeProto::INT,
            static_cast<int64_t>(onnx::TensorProto_DataType_FLOAT))
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
          propagateShapeAndTypeFromFirstInput(ctx);
          auto stash_type = static_cast<int64_t>(onnx::TensorProto_DataType_FLOAT);
