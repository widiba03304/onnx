...
class OpSetID final {
 ...
 public:
  ...
  std::string toString() const {
    return domain_ + "$" + onnx::to_string(version_);
  }
  ...
  static OpSetID fromString(const std::string& target) {
    ONNX_TRY {
      ...
      int new_version = onnx::stoi(target.substr(target.find("$") + 1, target.length()).c_str());
 ...
 public:
  ...
  void addInitializer(Tensor& initializer) {
    if (initializer.name().empty()) {
      initializer.setName(onnx::to_string(getNextUnique()));
    }
  ...
  size_t getNextUnique() {
    std::string next_unique_name = onnx::to_string(++next_unique_);
    while (!isNameUnique(next_unique_name)) {
      next_unique_name = onnx::to_string(++next_unique_);
    }
    return next_unique_;
  }
  ...
  Value* addInitializerAndInput(const Tensor& initializer) {
    return addInitializerAndInput(initializer, onnx::to_string(getNextUnique()));
  }
...
inline void Value::replaceAllUsesWith(Value* newValue) {
  ...
  if (this->elemType() != onnx::TensorProto_DataType_UNDEFINED) {
  ...
  if (std::find(graph->outputs().rbegin(), graph->outputs().rend(), this) != graph->outputs().rend()) {
    ...
    this->setUniqueName(onnx::to_string(graph->getNextUnique()), false);
  }
